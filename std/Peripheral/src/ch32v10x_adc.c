 ...
 * @return  none
 */
void ADC_Init(ADC_TypeDef * const ADCx, const ADC_InitTypeDef * const ADC_InitStruct)
{
    uint32_t tmpreg1 = 0;
    uint8_t tmpreg2 = 0;
...
void ADC_RegularSequencerLengthConfig(ADC_TypeDef * const ADCx, const uint8_t Length)
{
    uint32_t tmpreg1 = 0;
    uint8_t tmpreg2 = 0;
...
/* my variant */
void ADC_RegularChannelConfig(ADC_TypeDef * const ADCx,
                              const uint8_t ADC_Channel,
                              const uint8_t Rank,
                              const uint8_t ADC_SampleTime)
{
    {
        /**
         * [0..9] - ADCx->SAMPTR2 - offset 1
         * [10..17] - ADCx->SAMPTR1 - offset 0
         */
        const u8 offset = (1 - (ADC_Channel / 10));
        const u8 pos_in_group = (ADC_Channel % 10);
        const u8 shift = (3 * pos_in_group);

        volatile u32 * smartp = (&ADCx->SAMPTR1) + offset;
        const u32 smartp_now = (*smartp) & ~(SAMPTR1_SMP_Set << shift);
        const u32 smartp_new = smartp_now | (ADC_SampleTime << shift);
    ...
    {
        /**
         * [0..5] - ADCx->RSQR3 - offset 2
         * [6..12] - ADCx->RSQR2 - offset 1
         * [13..15] - ADCx->RSQR1 - offset 0
         */
        const u8 offset = (2 - (Rank / 6));
        const u8 pos_in_group = (Rank % 6);
        const u8 shift = (5 * pos_in_group);

        volatile u32 * rsqr = (&ADCx->RSQR1) + offset;
        const u32 rsqr_now = (*rsqr) & ~(RSQR3_SQ_Set << shift);
        const u32 rsqr_new = rsqr_now | (ADC_Channel << shift);

        *rsqr = rsqr_new;
    }
}

void ADC_RegularDrop(ADC_TypeDef * const ADCx)
{
    ADCx->RSQR1 = 0;
    ADCx->RSQR2 = 0;
    ADCx->RSQR3 = 0;
 ...
 * @return  none
 */
void ADC_TempSensorVrefintCmd(ADC_TypeDef * const ADCx, FunctionalState NewState)
{
    if(NewState != DISABLE)
    {
        ADCx->CTLR2 |= CTLR2_TSVREFE_Set;
    }
    else
    {
        ADCx->CTLR2 &= CTLR2_TSVREFE_Reset;
    }
}
...
{
    const uint32_t enablestatus = (ADCx->CTLR1 & (uint8_t)ADC_IT);
    const uint32_t itstat = (ADCx->STATR & (ADC_IT >> 8));
    return (0 < itstat);
}
