 ...
 * @return  none
 */
void NVIC_Init(const NVIC_InitTypeDef * const NVIC_InitStruct)
{
	const uint8_t NVIC_IRQChannel = NVIC_InitStruct->NVIC_IRQChannel;
	NVIC_SetPriority(NVIC_IRQChannel, NVIC_InitStruct->Priority);
    if(ENABLE == NVIC_InitStruct->NVIC_IRQChannelCmd) {
        NVIC_EnableIRQ(NVIC_IRQChannel);
    } else {
        NVIC_DisableIRQ(NVIC_IRQChannel);
    }
}
