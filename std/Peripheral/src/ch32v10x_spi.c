 ...
 * @return  none
 */
void SPI_Init(SPI_TypeDef * const SPIx, const SPI_InitTypeDef * const SPI_InitStruct)
{
    uint16_t tmpreg = 0;
 ...
 */
FlagStatus SPI_I2S_GetFlagStatus(const SPI_TypeDef * const SPIx,
                                 const uint16_t SPI_I2S_FLAG)
{
    return (0 < (SPIx->STATR & SPI_I2S_FLAG));
}
