#define NET_DEV_EDIT_COLOR_ST
#define STATE_NAME		dedit_color_st
...
GS_SCREEN(dedit_color, NULL, NULL);
...
void update(void)
{
	...
	const u8 * const line0 = TXT(TXT_NMED_COLOR);
    dedit_color_screen.lines[0] = line0;
    dedit_color_screen.lens[0] = 8;

    const u8 * const line1 = TXT(TXT_BLUE + SDATA->index);
    dedit_color_screen.lines[1] = line1;
    dedit_color_screen.lens[1] = strnlen(line1, 8);

    G_DRAWSCREEN(dedit_color_screen);
}
...
EPILOG()
{
	...
	if(SDATA->index != SDATA->edev->color){
		SDATA->edev->color = SDATA->index;
		cfg_storage_sync();
	}
}
