#define ALARM_TIME_ST
/*
 * edit system time state
 */

#define STATE_NAME		alarm_time_st
...
PROLOG()
{
    ...
    const u8_u8 curr_time = clcok_alarm_get_time(CLOCK_ALARM_FIRST);

	SDATA->params.hour = curr_time.u8_0;
	SDATA->params.min = curr_time.u8_1;
...
EPILOG()
{
    const bcd_time_t time = {
        .hour = bin8_to_bcd8(SDATA->params.hour),
        .min = bin8_to_bcd8(SDATA->params.min),
        .sec = 0,
    };
    const alarm_days_t ad = {
        .raw = 0xff,
    };
    clock_alarm_set(CLOCK_ALARM_FIRST, &time, ad);
}
