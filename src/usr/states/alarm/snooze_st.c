#define ALARM_SNOOZE_ST
/*
 * edit long interval
 */
#define STATE_NAME		alarm_snooze_st
...
GS_SCREEN(edit_snooze, NULL, long_str);
...
void update(void)
{
    ...
    *dst++ = '0' + (value % 10);
    ...
    const u8 * const line0 = TXT(TXT_AM_SNOOZE);
    edit_snooze_screen.lines[0] = line0;
    edit_snooze_screen.lens[0] = 8;

    const u8 * const line1 = long_str;
    edit_snooze_screen.lines[1] = line1;
    edit_snooze_screen.lens[1] = strnlen(line1, 8);

    G_DRAWSCREEN(edit_snooze_screen);
}
...
{
	SDATA->timeout = TIMEOUT;
    SDATA->index = config.dev_common.snooze;
	update();
}
...
EPILOG()
{
	if(SDATA->index != config.dev_common.snooze){
		config.dev_common.snooze = SDATA->index;
		cfg_storage_sync();
	}
}
